...
class MathReasoningProblem(MergingProblem):
    def __init__(self, merger, search_df, test_df, lang_id, conf_pe, lm_eval_tasks=None,
                 n_var=11, n_obj=2, n_eq_constr=0, n_ieq_constr=0, xl=0.001, xu=1, eval_batch_size=5,
                 detect_lang=True, additional_templates_folder=None, tokenizer=None,  
                 **kwargs):
    ...
    def metrics_4_genotype(self, model, tokenizer=None):
        ...
        if not self.use_lm_eval:
            df["predictions"] = get_batched_model_predictions(
                model, tokenizer, df, batch_size=self.eval_batch_size,
                max_token=256, device=device
            )
            ...
            evaluator = LmHarnessEvaluator(
                task_name=self.lm_eval_tasks["test" if self.test_mode else "search"][self.lang_id],
                sample_ids=self.conf_pe.sample_ids,
                correctness_metric=self.conf_pe.correct_metric,
                lang_id=self.lang_id if self.detect_lang else None,
                is_test=self.test_mode,
            ...
            correctness = evaluator.evaluate(model)
            data = evaluator.get_data()
            predictions = data["model_answers"]
    
        acc = PerformanceEstimator(PerformanceEstimationParameters(
            thetas=self.conf_pe.thetas,
            sample_weights=self.conf_pe.weights,
            sample_ids=self.conf_pe.sample_ids,
            mode=self.conf_pe.mode if not self.test_mode else "mean",
            bench=self.conf_pe.bench
        )).estimate_accuracy(correctness)
        
        f1 = -acc

        tok = tokenizer or self.tokenizer
        assert tok is not None, "Tokenizer required for response length computation."

        lengths = [len(tok.encode(str(p), add_special_tokens=False)) for p in predictions]
    
        f2 = np.mean(lengths)
        print(f"Fitness: acc={acc:.3f}, len_avg={f2:.1f}")
        
        return [f1, f2], f"Fitness: acc={acc:.3f}, len={f2:.3f}"

    def test(self, genotype, base_model: Optional[Path] = None):
        self.test_mode = True
        if base_model:
            model = self.load_model(base_model)
            tokenizer = None if self.use_lm_eval else model[1]
        else:
            path_to_model = self._from_array_to_genotype(genotype)
            model = self.load_model(path_to_model)
            tokenizer = None if self.use_lm_eval else model[1]
        ...
        return self.metrics_4_genotype(model[0] if not self.use_lm_eval else model, tokenizer)